...
async function SuspenseBoundary({
  ...
  const { pricingRows, receiptUrl } = await getStripeDetails(
  ...
  return (
    <>
      ...
      <Card>
        ...
        <CardContent className='pb-4 grid grid-col-1 sm:grid-cols-2 gap-4 border-t pt-4'>
          <div>
...
async function getStripeDetails(
  ...
  const { payment_intent, total_details, amount_total, amount_subtotal, } = await stripeServerClient.checkout.sessions.retrieve(
    sessionId, {
    expand: [
  ...
  const refundAmount = getRefundAmount(payment_intent, isRefunded, pricePaidInCents);
...
function getRefundAmount(
  ...
  if (typeof payment_intent !== 'string'
    ...
    return payment_intent?.latest_charge?.amount_refunded;
  }

  // in case stripe doesn't have the refund info and we have marked the purchase as refunded, use the price
  return isRefunded ? pricePaidInCents : 0;
}
